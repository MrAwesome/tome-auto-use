...
local function spotHostiles(self)
    ...
    core.fov.calc_circle(self.x, self.y, game.level.map.w, game.level.map.h, self.sight or 10,
        function(_, x, y) return game.level.map:opaque(x, y) end, function(_, x, y)
...
function _M:automaticTalents()
        ...
        if t then
            ...
            if auto_use == 1 and (t.mode ~= "sustained" or not self.sustain_talents[tid]) and not self.talents_cd[tid] and self:preUseTalent(t, true, true) and (not t.auto_use_check or t.auto_use_check(self, t)) and haselites < 1 then
                ...
                if minty == 1 and rangedmax == 0 and rangedtwotiles == 0 then
                    uses[#uses + 1] = { name = t.name, no_energy = t.no_energy == true and 0 or 1, cd = self
                    :getTalentCooldown(t) or 0, tid = tid, is_item = inventory, item_name = item_name, ftarget =
                    lc_target }
                end
    ...
    if game.player.talents_auto_order and not game.player.talents_auto_ordering_off then
        ...
        table.sort(temp_uses, function(a, b)
            if a.no_energy < b.no_energy then
                return true
            elseif a.no_energy > b.no_energy then
                return false
            else
                if a.cd > b.cd then
                    return true
